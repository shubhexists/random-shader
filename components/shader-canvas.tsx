...
export default function ShaderCanvas({
  ...
  const initGL = useCallback(() => {
    ...
    try {
      ...
      colorMultiplierLocationRef.current = gl.getUniformLocation(
        program,
        "iColorMultiplier"
      );
      colorShiftLocationRef.current = gl.getUniformLocation(
        program,
        "iColorShift"
      );
      distortionLocationRef.current = gl.getUniformLocation(
        program,
        "iDistortion"
      );
  ...
  const renderGL = useCallback(() => {
    ...
    if (intensityLocationRef.current)
      gl.uniform1f(intensityLocationRef.current, parameters.intensity);
    if (scaleLocationRef.current)
      gl.uniform1f(scaleLocationRef.current, parameters.scale);
    if (colorMultiplierLocationRef.current)
      gl.uniform1f(
        colorMultiplierLocationRef.current,
        parameters.colorMultiplier
      );
    if (colorShiftLocationRef.current)
      gl.uniform1f(colorShiftLocationRef.current, parameters.colorShift);
    if (distortionLocationRef.current)
      gl.uniform1f(distortionLocationRef.current, parameters.distortion);
